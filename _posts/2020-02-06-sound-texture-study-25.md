---
layout: post
title:  "Sound Texture Study 25"
author: naoto
categories: [ sketch ]
tags: [ p5js, sound ]
image: assets/images/2020-02-06-sound-texture-study-25.png
...
class Cube {
  ...
  draw(pg, args) {
    ...
    if (mode == 'stroke') {
      pg.noFill();
      pg.strokeWeight(width * 0.01 * fading);
    }
    else {
      pg.noStroke();
    }
...
const s = (p) => {
  ...
  const history = [];
  const savedHistory = ["1<<<<<<+1p>>>>>>", "d1<<<<<+1p>>>>>====b", "d1<<<<<+1p>>>>>", "d1<<<<<+1m>>p>>>", "d1<<<<<+1m>>p>>~>", "d1<<<n<<+1m>>p>>~>", "d1<<<n<<+1m>>p>*0.5>~>", "d10<<<n<<+1m>>p>>~>", "d10<<<n<<+1m>>p>>~>[", "d10<<<n<f<+1m>>p>>~>[", "d10<<<n<f<+1N>>p>>~>[", "d10<<<n<f<+1=>>p>>~>[", "d10<<<<f<+1=>>p>>~>["]
  let curHistory = 0;
