---
layout: post
title:  "Sound Algorithm Study 8"
author: naoto
categories: [ sketch ]
tags: [ p5js, sound ]
image: assets/images/2020-01-01-sound-algorithm-study-8.png
...
class WipeGraphics extends LayerGraphics {
  ...
  update({ t }) {
    ...
    this.foreDraw.draw({ pg: this.pgF });
    setColorMode = 2;
    this.wipeDraw.draw({ pg: this.pgMask, args: { tw: this.bangTween, bangParam: this.bangParam } });
    setColorMode = 0;
  ...
  setup() {
    ...
    for (let i = 0; i < this.numBlobs; i++) {
      this.circlePaths.push({
        ...
        radius: i == 0 ? 120 : p.random(100, 120),
...
const s = (p) => {
  ...
  p.setup = () => {
    ...
    wipe0 = new WipeGraphics({
      p,
      fore: new Drawer({ c: solidDraws[1] }),
      back: new Drawer({ c: solidDraws[0] }),
      wipe: new Drawer({ c: wipeDraws[0], args: { wipe: wipe0 } })
    });
    // wipe0.wipeDraw = new Drawer({ c: solidDraws[2], args: { wipe: wipe0, col: {fg:0, bg: 2} } });
    // wipe0.wipeDraw = new Drawer({ c: solidDraws[3], args: { wipe: wipe0, col: {fg:0, bg: 2} } });
